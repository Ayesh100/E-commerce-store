    ...
    <div className="container mt-4">
      ...
      {loading ? (
        <div className="text-center">
          <ClipLoader size={40} color={"#0D6EFD"} loading={loading} />
        </div>
      ) : (
        <>
          <p><strong>Order ID:</strong> {orderDetails?.order?.id}</p>
          <p><strong>Total Price:</strong> Rs.{orderDetails?.order?.total_price}</p>
          <p><strong>Order Date:</strong> {new Date(orderDetails?.order?.created_at).toLocaleDateString()}</p>
          <p>
